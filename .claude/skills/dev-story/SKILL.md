...
- Communicate all responses in plain English prose, tailored to {user_skill_level}
...
<workflow>
  <critical>Communicate all responses in plain English prose, tailored to {user_skill_level}</critical>
  ...
  <step n="10" goal="Completion communication and user support">
    ...
    <action>Recommended next steps (flexible based on project setup):
      ...
      - Optional: run `testarch-trace` to verify coverage and emit the per-story gate decision
    </action>
  ...
  <step n="11" goal="Write the STORY-DONE completion contract (MANDATORY)">
    <critical>This step is MANDATORY and never skipped, paused, or deferred — the merge gate validates the STORY-DONE file mechanically, and an invalid or missing STORY-DONE blocks the merge.</critical>

    <action>Write the STORY-DONE completion contract to `_bmad-output/implementation-artifacts/done/story-{id}-done.md` (where `{id}` is this story's id, e.g. `1-2`; create the `done/` directory if needed) using the schema in `{skill-root}/story-done-template.md`. Frontmatter, all fields required:
      - `type: story-done`
      - `story_id` — this story's id
      - `epic` — the epic number
      - `status: completed`
      - `completed_at` — ISO-8601 UTC timestamp (from `date -u +%Y-%m-%dT%H:%M:%SZ`, never invented)
      - `commits: [real SHAs on this branch]` — taken from `git log`, every SHA must exist in this repository
      - `tests: {passed: true, count: N}` — from the fresh full-suite run in Step 9; N is the passing test count, an integer > 0
      - `touches: [files actually modified]` — mirror the story's File List, paths relative to repo root
    </action>
    <action>Commit the STORY-DONE file to this branch (e.g. `git add _bmad-output/implementation-artifacts/done/story-{id}-done.md && git commit -m "chore: STORY-DONE contract for story {id}"`)</action>
    <action>Verify the contract locally when the gate script is available: `python3 <gates>/story_done_check.py --story {id}` must exit 0; fix any reported errors before finishing</action>

    <action if="tests did not pass or no real commit SHAs exist">HALT - a STORY-DONE contract must never be written with invented data; fix the underlying state first</action>

    <output>📜 STORY-DONE contract written and committed: `_bmad-output/implementation-artifacts/done/story-{id}-done.md`
      The merge gate validates this file mechanically — an invalid or missing STORY-DONE blocks the merge.
    </output>
  <action>If the resolved `{workflow.on_complete}` value is non-empty, follow it as the final terminal instruction before exiting.</action>
  </step>
