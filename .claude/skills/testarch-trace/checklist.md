...
- [ ] Story file is accessible (if tracing a story)
...
- [ ] `gate-decision-{story_id}.json` written to `{gate_decision_output}` when gate-eligible (per-story filename — never a shared `gate-decision.json`)
- [ ] `gate-decision-{story_id}.json` contains `evaluated_at`, `gate_basis`, `gate_status`, `rationale`, and per-criterion status fields
...
- [ ] `gate-decision-{story_id}.json` saved to `{gate_decision_output}` (when gate-eligible)
