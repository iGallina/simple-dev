...
**Optional: emit `gate-decision-{story_id}.json`** for pipelines that only need the gate signal without the full summary. The filename is per-story — `{story_id}` is the trace target id — because a single overwritten `gate-decision.json` lets a stale PASS gate the wrong story:
...
if (gateEligible && ['PASS', 'CONCERNS', 'FAIL', 'WAIVED'].includes(gateDecision)) {
  ...
  // Per-story gate file: {story_id} = trace target id (e.g. "1-2-user-auth").
  const storyId = String((e2eTraceSummary.target && e2eTraceSummary.target.id) || 'unscoped').replace(/[^A-Za-z0-9._-]/g, '-');
  const gateDecisionPath = '{gate_decision_output}'.replace('{story_id}', storyId);
  fs.writeFileSync(gateDecisionPath, JSON.stringify(gateDecisionSlim, null, 2), 'utf8');
  console.log(`✅ gate-decision-${storyId}.json written to ${gateDecisionPath}`);
}
```
...
- ✅ `gate-decision-{story_id}.json` written to `{gate_decision_output}` (when gate-eligible)
...
- `gate-decision-{story_id}.json` written when gate-eligible
